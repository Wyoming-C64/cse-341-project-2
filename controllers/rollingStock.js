...
const rollingStockCollection = 'rollingStock';
...
const getAll = async (req, res, next) => {
  ...
  */
  ...
  const result = await mongoDb.getDb()
    .db()
    .collection(rollingStockCollection)
    .find();
...
const putData = async (req, res, next) => {
  ...
  /*  #swagger.summary = 'Update a single roster record.'
      ...
      #swagger.responses[204] = {
        description: "Success - The roster record for a single piece of rolling stock is updated with the new data. No data is returned other than this status.",
      }
      #swagger.responses[404] = {
        description: "Not found.",
      }
  ...
  */
  ...
  const dbResult = mongoDb.getDb()
    .db()
    .collection(rollingStockCollection)
    .findOneAndUpdate( {"_id": myObjId }, {$set: record} );   
  dbResult.then( 
    (resultData) => {
